...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // stream tokens as they arrive; sources ride in a trailer line
    res.statusCode = 200;
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (evt.type === 'content_block_delta' && evt.delta && evt.delta.text) {
            const clean = evt.delta.text.replace(/\u2014/g, ', ').replace(/\u2013/g, '-');  // no em dashes, swept live
            full += clean;
            res.write(clean);
          }
        } catch {}
    ...
    clearTimeout(timer);
    res.write('\n\n@@JEPMETA@@' + JSON.stringify({ sources: found.map(p => p.url).filter(Boolean), ms: { total: Date.now() - t0 } }));
    return res.end();
  } catch (e) {
